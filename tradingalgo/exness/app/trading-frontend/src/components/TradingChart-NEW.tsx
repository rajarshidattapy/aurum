...
export function TradingChart({ currentPrice, symbol, className = '' }: TradingChartProps) {
  ...
  // SINGLE CLEAN data generation function
  const generateCandleData = (count: number, interval: string) => {
    ...
    // Generate ONE CLEAN dataset with NO duplication
  ...
  // Generate SINGLE set of candlesticks
  const candlesticks = candleData.map((candle, index) => {
    ...
    const candleWidth = Math.min(candleSpacing * 0.7, 12)
  ...
  const handleMouseMove = (event: React.MouseEvent<SVGSVGElement>) => {
    ...
    const index = Math.floor(((x - padding) / (chartWidth - 2 * padding)) * candleData.length)
    const candle = candleData[index]
    
    if (candle) {
      setHoverData({ x, candle, visible: true })
    }
  }
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      {/* SINGLE Chart Container - NO DUPLICATION */}
      <div className="w-full bg-card rounded-lg border border-border p-4 relative">
        ...
        {/* SINGLE SVG Chart - GUARANTEED NO DUPLICATION */}
        <div className="relative w-full bg-[#0a0e27] rounded-lg border border-gray-700" style={{ height: '400px' }}>
          ...
          >
            ...
            {/* SINGLE SET OF CANDLESTICKS - NO DUPLICATION */}
